    ...
    db.child("patients").on("value", snap => {
      snap.forEach(patient => {
        if (cluster.has(patient.key)) {
          let tmp = patient.val();
          document.getElementById("tr_" + patient.key).setAttribute("data-image", tmp.img);
          document.getElementById("span2_" + patient.key).innerHTML = tmp.name;
          document.getElementById("span3_" + patient.key).innerHTML = String(((1.0 * tmp.sum / tmp.total) * 100).toFixed(2)) + '%';
          document.getElementById("span4_" + patient.key).innerHTML = '-' + (tmp.total - tmp.sum) + '$';
        }
      });
    });
  }, function (err) {
